...
export default {
  ...
  methods: {
    getAllMovies3() {
      this.$axios
        .get("http://localhost:8000/api/page/3")
        .then(response => {
          this.contents3 = response.data.results;
          this.getAllGenres();
        })
        .catch(error => {
    ...
    filteredList() {
      if (this.searchTitle == "") {
        this.contents3 = this.contents3.filter(contents3 => {
          return contents3.genre_ids
            .toString()
            .toLowerCase()
        ...
        this.contents3 = this.contents3.filter(contents3 => {
          return contents3.original_title
            .toLowerCase()
